(function($) {
	// Create namespace object and add to window (global).
	if(typeof window.cms === 'undefined') {window.cms = {};}

	/** Set document orientation. */
	function setDocumentOrientation(intValue) {
		// Validate orientation.
		switch(intValue) {
			default: case 0: // Portrait.
				if(!$(document.body).hasClass('orientation_portrait')) {
					$(document.body).removeClass('orientation_landscape').addClass('orientation_portrait');
				}
				break;
			case 90: case -90: // Landscape.
				if(!$(document.body).hasClass('orientation_landscape')) {
					$(document.body).removeClass('orientation_portrait').addClass('orientation_landscape');
				}
				break;
		}
	}

	// Validate default orientation from document and add to global object.
	if(typeof window.orientation !== 'undefined') {
		window.cms.intOrientation = window.orientation;
	} else if($(document.body).hasClass('orientation_portrait')) {
		window.cms.intOrientation = 0;
	} else if($(document.body).hasClass('orientation_landscape')) {
		window.cms.intOrientation = 90;
	}

	// Validate global object from window.
	if(typeof window.cms.intOrientation !== 'undefined') {
		// Init.
		var objSupportsOrientationChange = 'onorientationchange' in window, objOrientationEvent = objSupportsOrientationChange ? 'orientationchange' : 'resize';
		// Add orientation event to global object.
		window.cms.objOrientationEvent = objOrientationEvent;
		// Set document orientation.
		setDocumentOrientation(window.cms.intOrientation);
		// Bind event.
		$(window).bind(objOrientationEvent, function(evt) {
			setDocumentOrientation(window.orientation);
		});
	}
})(jQuery);
...
(function($) {
	// Validate full-screen mode.
	if(navigator.standalone) {
		// Navigate like native application.
		$('a:internal').live('click', function(evt) {
			evt.preventDefault();
			window.location = $(this).attr('href');
		});
	} else { // Not in full-screen.
		// Init default behavior for external links.
		$('a:external').not('.share a').addClass('external').bind('click', function(evt){
			return !window.open(this.href);
		});
		// Hide URL bar on mobile only.
